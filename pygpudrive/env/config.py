...
class EnvConfig:
    ...
    steer_actions: torch.Tensor = torch.round(torch.linspace(-1.0, 1.0, 13) * 1000) / 1000
    accel_actions: torch.Tensor = torch.round(torch.linspace(-4.0, 4.0, 7) * 1000) / 1000
    head_tilt_actions: torch.Tensor = torch.Tensor([0])

    # Delta Local dynamics model
    dx: torch.Tensor = torch.round(torch.linspace(-2.0, 2.0, 20) * 1000) / 1000
    dy: torch.Tensor = torch.round(torch.linspace(-2.0, 2.0, 20) * 1000) / 1000
    dyaw: torch.Tensor = torch.round(torch.linspace(-3.14, 3.14, 20) * 1000) / 1000

    # Global action space settings if StateDynamicsModel is used
    x: torch.Tensor = torch.round(torch.linspace(-100.0, 100.0, 10) * 1000) / 1000
    y: torch.Tensor = torch.round(torch.linspace(-100.0, 100.0, 10) * 1000) / 1000
    yaw: torch.Tensor = torch.round(torch.linspace(-3.14, 3.14, 10) * 1000) / 1000
    vx: torch.Tensor = torch.round(torch.linspace(-10.0, 10.0, 10) * 1000) / 1000
    vy: torch.Tensor = torch.round(torch.linspace(-10.0, 10.0, 10) * 1000) / 1000
